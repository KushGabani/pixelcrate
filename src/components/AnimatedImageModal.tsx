...
const AnimatedImageModal: React.FC<AnimatedImageModalProps> = ({
  ...
  useEffect(() => {
    if (selectedImage && initialPosition && !optimalDimensions) {
      const fallback = {
        ...
        top:
          window.innerHeight / 2 - Math.min(600, window.innerHeight * 0.8) / 2,
        left:
          window.innerWidth / 2 - Math.min(800, window.innerWidth * 0.8) / 2,
      };
  ...
  const modalVariants = useMemo(() => {
    ...
    if (!initialPosition || !finalDimensions) {
      return null;
    }
    ...
    const initialX =
      initialPosition.left - containerCenter + initialPosition.width / 2;
    ...
    const isTallImage =
      selectedImage && selectedImage.height > selectedImage.width * 2;

    // For tall images, keep top alignment with 40px padding
    // For regular images, add moderate top padding (20px)
    const finalY = isTallImage
      ? 40
      : Math.max(20, (window.innerHeight - finalDimensions.height) / 2);

    // Create optimized transitions
    const openTransition = createOptimizedTransition(
      prefersReducedMotion,
      "spring",
    );
    const exitTransition =
      zoomState.scale > 1
        ? createOptimizedTransition(prefersReducedMotion, "spring")
        : createOptimizedTransition(prefersReducedMotion, "spring");
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            {...createFadeVariants(prefersReducedMotion)}
            onClick={handleClose}
            style={{
          ...
          <div
            ...
            style={{
              overflow:
                selectedImage.height > selectedImage.width * 2
                  ? "auto"
                  : "visible", // Enable scrolling for tall images
              paddingTop: 0,
              paddingBottom:
                selectedImage.height > selectedImage.width * 2
                  ? "40px"
          ...
          >
            ...
            <motion.div
              ...
              style={{
                ...
                marginBottom: 0,
                paddingBottom:
                  selectedImage.height > selectedImage.width * 2 ? "30px" : 0,
                height: "fit-content",
                width: "fit-content",
            ...
            >
              <ZoomableImageWrapper
                ...
                onLoad={(e) => {}}
                onClose={handleClose}
                onZoomStateChange={(scale, position) =>
                  setZoomState({ scale, position })
                }
