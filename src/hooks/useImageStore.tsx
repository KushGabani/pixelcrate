...
export function useImageStore() {
  ...
  const updateImage = useCallback(
    (id: string, updater: (img: ImageItem) => ImageItem) => {
      collection.setImages((prevImages) =>
        prevImages.map((img) => (img.id === id ? updater(img) : img)),
      );
    },
    ...
    async (file: File) => {
      await fileSystem.addImageFromFile(
        ...
        async (media, dataUrl, savedFilePath) => {
          ...
          // Perform analysis
          const analyzedMedia = await analysis.analyzeAndUpdateImage(
            analyzingMedia,
            dataUrl,
            savedFilePath,
          );
  ...
  const importFromFilePath = useCallback(
